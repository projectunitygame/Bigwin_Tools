...
namespace CMS_Tools.Model
{
    public class AccountModel
    {
                    ...
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        if (code == 1)
                        {
                            return new UserInfo()
                            {
                                ...
                                MasterID = dt.Rows[0]["LoginId"].ToString()
                            };
                        }
                        else
                        {
        ...
        public UserInfo LoginWithAccountID(int accountID,string masterID, string ip, ref int? code)
        {
            try
            {
                code = -1;
                using (SqlConnection connection = new SqlConnection(connectString2))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Parameters.Add("@Code", SqlDbType.Int);
                        cmd.Parameters["@Code"].Direction = ParameterDirection.Output;
                        cmd.Parameters.Add("@TokenNew", SqlDbType.VarChar, 32);
                        cmd.Parameters["@TokenNew"].Direction = ParameterDirection.Output;
                        ...
                        code = int.Parse(cmd.Parameters["@Code"].Value.ToString());
                        string tokenNew = cmd.Parameters["@TokenNew"].Value.ToString();
                        if (code == 1)
                        {
                            return new UserInfo()
                            {
            ...
            {
                Logs.SaveError("Error LoginWithAccountID: " + ex, ex);
                throw new Exception(ex.Message);
            }
        }
    ...
    public class UserInfo
    {
        ...
        public Apis.AgencyInfoEntity AgencyInfo  { get; set; }
    }
}